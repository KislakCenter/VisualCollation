RailsJwtAuth.setup do |config|
  ...
  # mailer class name
  config.mailer_name = 'AuthenticationMailer'

  # mailer sender
  config.mailer_sender = 'emeryr@upenn.edu'

  # activate email notification when email is changed
  config.send_email_change_requested_notification = false

  # activate email notification when password is changed
  config.send_password_changed_notification = false
  ...
  config.confirm_email_url = "https://#{ENV['APPLICATION_HOST']}/confirmations"

  # url used to create email link with reset password token
  config.reset_password_url = "https://#{ENV['APPLICATION_HOST']}/reset_passwords"
