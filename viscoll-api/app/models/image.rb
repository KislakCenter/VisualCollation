...
class Image
  ...
  validates :filename, uniqueness: { message: "Image with filename: '%<value>s', already exists.", scope: :user }

  protected

  # If linked to side(s), remove link from the side(s)
  def unlink_sides_before_delete
    sideIDs.each do |sideID|
  ...
  def delete_file
    path = "#{Rails.root}/public/uploads/#{fileID}"
