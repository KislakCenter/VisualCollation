    ...
    def handleMappingImport(newProject, imageData, current_user)
        ...
        Zip::File.open(tempzip.path) do |zip_file|
          zip_file.each do |file|
            ...
            uploadedImages["#{filename}.#{extension}"] =
              { fileID: fileID, file: tempfile, extension: extension }
          end
        end
      end
      ...
      newProject.sides.each do |side|
        ...
        if !image
          ...
          if uploadedImages.key?(filename)
            ...
            if existingImage
              ...
              if uploadedImages[filename][:fileID] == existingImage.fileID
                ...
                side.image['url'] = "#{@base_api_url}/images/#{existingImage.id}_#{existingImage.filename}"
                side.save
                ...
                uploaded_file = uploader.upload(uploadedImages[filename][:file],
                                                metadata: { 'filename' => newFilename,
                                                            "mime_type": "image/#{uploadedImages[filename][:extension]}" })
                newImage = Image.new(user: current_user, filename: newFilename, fileID: uploaded_file.id,
                                     metadata: uploaded_file.metadata, projectIDs: [newProject.id.to_s])
                side.image['url'] = "#{@base_api_url}/images/#{newImage.id}_#{newFilename}"
                side.save
              ...
              uploader = Shrine.new(:store)
              uploaded_file = uploader.upload(uploadedImages[filename][:file],
                                              metadata: { 'filename' => filename.to_s,
                                                          "mime_type": "image/#{uploadedImages[filename][:extension]}" })
              newImage = Image.new(user: current_user, filename: filename, fileID: uploaded_file.id,
                                   metadata: uploaded_file.metadata, projectIDs: [newProject.id.to_s])
              side.image['url'] = "#{@base_api_url}/images/#{newImage.id}_#{newImage.filename}"
              side.save
              !newImage.sideIDs.include?(side.id.to_s) ? newImage.sideIDs.push(side.id.to_s) : nil
              !newImage.projectIDs.include?(newProject.id.to_s) ? newImage.projectIDs.push(newProject.id.to_s) : nil
              newImage.save
          ...
          side.image['url'] = "#{@base_api_url}/images/#{image.id}_#{image.filename}"
          side.save
