  ...
  module ProjectValidationHelper
    def validateProjectCreateGroupsParams(allGroups)
      ...
      allGroups.each_with_index do |group, index|
        ...
        if leaves.is_a?(Integer) && leaves.odd?
        ...
        unless conjoin.is_a?(Boolean)
          @group_error[:conjoin].push('should be a Boolean')
          haveGroupError = true
        end
        @group_errors.push(@group_error) if haveGroupError
      end
      { status: @group_errors.empty?, errors: @group_errors }
    end
  end
end
