class SidesController < ApplicationController
  ...
  def update
    if !@side.update(side_params)
      raise VCError.new("Side (#{@side.id} could not be updated: #{@side.errors.full_messages.join "\n"})", status: :unprocessable_entity)
    end
  end
  ...
  def updateMultiple
    ...
    if haveErrors
      raise VCError.new("Errors occurred when updating: #{@errors.join "\n"}", status: :unprocessable_entity)
    end
    allSides.each_with_index do |side_params, index|
      side              = sides[index]
      previousSideImage = side.image.clone
      if !side.update(side_params[:attributes])
        raise VCError.new("Errors occurred when updating sides: #{side.errors.full_messages.join "\n"}", status: :unprocessable_entity)
      else
