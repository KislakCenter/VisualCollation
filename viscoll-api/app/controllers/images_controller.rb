...
class ImagesController < ApplicationController
  before_action :authenticate!, except: %i[show getZipImages]

  def authorize_image!(image)
    return if current_user.id == image.user_id

    raise ApplicationController::VCError,
          "Image (#{image.id}) is not authorized for current user (#{current_user.id}); expected: '#{image.user_id}'."
  end
  ...
  def uploadImages
    ...
    allImages.each do |image_data|
      ...
      image         = Image.new(user: current_user, filename: "#{filename}.#{extension}", fileID: uploaded_file.id,
                                metadata: uploaded_file.metadata, projectIDs: projectIDs)
      if image.valid?
        image.save
      else
        copyCounter = 1
        until image.save
          if image.errors.key?('filename') && image.errors[:filename][0].include?('Image with filename')
  ...
  def link
    ...
    projects.each do |project|
      images.each do |image|
        unless image.projectIDs.include? project.id.to_s
  ...
  def image_create_params
    params.permit(:projectID, images: %i[filename content])
  end
