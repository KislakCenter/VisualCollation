...
class ExportController < ApplicationController
  ...
  def show
    ...
    current_user.images.all.each do |image|
      images.push(image) if image.projectIDs.include? @project.id.to_s
    end
    unless images.empty?
      basePath    = "#{Rails.root}/public/uploads/"
      zipFilename = "#{basePath}#{@project.id}_images.zip"
      ...
      @zipFilePath = "#{@base_api_url}/images/zip/#{@project.id}"
    end
    ...
    raise VCError, "Something went wrong when exporting #{@format}: #{errors}" unless errors.empty?

    case @format
    when 'xml'
      render json: { data: exportData, type: @format, Images: { exportedImages: @zipFilePath || false } },
    ...
    when 'svg'
      ...
      config_xml     = %(<config><css xml:id="css">#{collation_file}</css></config>)
      ...
      Zip::File.open(outfile) do |zip_file|
        zip_file.each do |entry|
          exportData << entry.get_input_stream.read.force_encoding('UTF-8') if File.extname(entry.name) === '.svg'
        end
      end
      ...
      Zip::OutputStream.open(outfile) do |zio|
        ...
        Zip::File.open_buffer StringIO.new(job_response.body).read do |zip_input|
          zip_input.each do |input_entry|
            ...
            next unless File.extname(input_entry.name) == '.svg'
    ...
    when 'formula'
      ...
      Zip::File.open(outfile) do |zip_file|
        zip_file.each do |entry|
          next unless File.basename(entry.name).include? 'formula'
    ...
    when 'html'
      ...
      render json: { data: exportData, type: 'formula', Images: { exportedImages: @zipFilePath || false } },
             status: :ok and return
    else
      raise VCError,
            "Export format must be one of [json, xml, svg, formula, html]. The format received is: '#{@format}'."
    end
  end
  ...
  def remove_xml_declaration(zip_file, input_file)
    ...
    new_content = content.lines.to_a[1..].join
  ...
  def add_doctype(zip_file, input_file)
    content = zip_file.read(input_file.name)
    zip_file.get_output_stream(input_file.name) do |f|
      f.puts "<!-- Generated with VCEditor -->\n<!DOCTYPE html>\n#{content}"
    end
    zip_file.commit
  end
  ...
  def process_pipeline(pipeline, xml_string, config_xml = nil, image_list = nil)
    ...
    Net::HTTP.start(job_uri.hostname, job_uri.port) do |http|
