...
describe 'PUT /images/unlink', type: :request do
  before do
    ...
    @image1 = FactoryGirl.create(:pixel, user: @user, projectIDs: [@project1.id.to_s, @project2.id.to_s],
                                         sideIDs: [@project1.leafs[0].rectoID, @project2.leafs[0].rectoID])
    @image2 = FactoryGirl.create(:shiba_inu, user: @user, projectIDs: [@project1.id.to_s, @project2.id.to_s],
                                             sideIDs: [@project1.leafs[0].versoID, @project2.leafs[0].versoID])
    @parameters = {
      "projectIDs": [],
      "imageIDs": []
    }
  ...
  context 'with valid authorization' do
    context 'and valid image and project' do
      before do
        ...
        put '/images/unlink', params: @parameters.to_json,
    ...
    context 'and valid image but missing project' do
      before do
        @parameters[:projectIDs] = [@project1.id.to_s, "#{@project2.id}missing"]
        @parameters[:imageIDs] = [@image1.id.to_s, @image2.id.to_s]
        put '/images/unlink', params: @parameters.to_json,
                              headers: { 'Authorization' => @authToken, 'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' }
        @image1.reload
    ...
    context 'and missing image but valid project' do
      before do
        @parameters[:projectIDs] = [@project1.id.to_s, @project2.id.to_s]
        @parameters[:imageIDs] = [@image1.id.to_s, "#{@image2.id}missing"]
        put '/images/unlink', params: @parameters.to_json,
                              headers: { 'Authorization' => @authToken, 'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' }
        @image1.reload
        @image2.reload
