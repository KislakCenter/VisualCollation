...
describe 'GET /projects/:id/export/:format', type: :request do
  before do
    stub_request(:get, 'https://digital.library.villanova.edu/Item/vudl:99213/Manifest').with(headers: { 'Accept' => '*/*', 'User-Agent' => 'Ruby' }).to_return(
      status: 200, body: File.read("#{File.dirname(__FILE__)}/../../fixtures/villanova_boston.json"), headers: {}
    )
    ...
    @project = FactoryGirl.create(:project,
                                  ...
                                  'taxonomies' => %w[Ink Unknown],
                                  'manifests' => { '12341234': { 'id' => '12341234',
                                                                 'url' => 'https://digital.library.villanova.edu/Item/vudl:99213/Manifest', 'name' => 'Boston, and Bunker Hill.' } })
    ...
    @upleafs = Array.new(2) do
      FactoryGirl.create(:leaf, project: @project, parentID: @testgroup.id.to_s, nestLevel: 1)
    end
    @testmidgroup = FactoryGirl.create(:group, project: @project, parentID: @testgroup.id.to_s, nestLevel: 2,
                                               title: 'Group 2')
    @midleafs = Array.new(2) do
      FactoryGirl.create(:leaf, project: @project, parentID: @testmidgroup.id.to_s, nestLevel: 2)
    end
    @botleafs = Array.new(2) do
      FactoryGirl.create(:leaf, project: @project, parentID: @testgroup.id.to_s, nestLevel: 1)
    end
    @botleafs[1].update(type: 'Endleaf')
    @project.add_groupIDs([@testgroup.id.to_s, @testmidgroup.id.to_s], 0)
    @testgroup.add_members(
      [@upleafs[0].id.to_s, @upleafs[1].id.to_s, @testmidgroup.id.to_s, @botleafs[0].id.to_s, @botleafs[1].id.to_s], 0
    )
    @testmidgroup.add_members([@midleafs[0].id.to_s, @midleafs[1].id.to_s], 0)
    @testterm = FactoryGirl.create(:term, project: @project, title: 'Test Term', taxonomy: 'Ink',
                                          description: 'This is a test', show: true, objects: { Group: [@testgroup.id.to_s], Leaf: [@botleafs[0].id.to_s], Recto: [@botleafs[0].rectoID], Verso: [@botleafs[0].versoID] })
    @testimage = FactoryGirl.create(:pixel, user: @user, projectIDs: [@project.id.to_s],
                                            sideIDs: [@upleafs[0].rectoID], filename: 'pixel.png')
  ...
  after :all do
    imagePath = "#{Rails.root}/public/uploads"
    File.delete("#{imagePath}/pixel") if File.file?("#{imagePath}/pixel")
  ...
  context 'with valid authorization' do
    context 'for JSON export' do
      before do
        @format = 'json'
        get "/projects/#{@project.id}/export/#{@format}",
      ...
      it 'has expected content' do
        ...
        expect(export_result['Groups']).to eq({
                                                '1' => { 'params' => { 'type' => 'Quire', 'title' => 'Group 1', 'nestLevel' => 1 }, 'tacketed' => [], 'sewing' => [],
                                                         'parentOrder' => nil, 'memberOrders' => %w[Leaf_1 Leaf_2 Group_2 Leaf_5 Leaf_6] },
                                                '2' => { 'params' => { 'type' => 'Quire', 'title' => 'Group 2', 'nestLevel' => 2 }, 'tacketed' => [], 'sewing' => [],
                                                         'parentOrder' => 1, 'memberOrders' => %w[Leaf_3 Leaf_4] }
                                              })
        expect(export_result['Leafs']).to eq({
                                               '1' => {
                                                 'params' => { 'folio_number' => '', 'material' => 'Paper', 'type' => 'Original', 'attached_above' => 'None',
                                                               'attached_below' => 'None', 'stub' => 'No', 'nestLevel' => 1 }, 'conjoined_leaf_order' => nil, 'parentOrder' => 1, 'rectoOrder' => 1, 'versoOrder' => 1
                                               },
                                               '2' => {
                                                 'params' => { 'folio_number' => '', 'material' => 'Paper', 'type' => 'Original', 'attached_above' => 'None',
                                                               'attached_below' => 'None', 'stub' => 'No', 'nestLevel' => 1 }, 'conjoined_leaf_order' => nil, 'parentOrder' => 1, 'rectoOrder' => 2, 'versoOrder' => 2
                                               },
                                               '3' => {
                                                 'params' => { 'folio_number' => '', 'material' => 'Paper', 'type' => 'Original', 'attached_above' => 'None',
                                                               'attached_below' => 'None', 'stub' => 'No', 'nestLevel' => 2 }, 'conjoined_leaf_order' => nil, 'parentOrder' => 2, 'rectoOrder' => 3, 'versoOrder' => 3
                                               },
                                               '4' => {
                                                 'params' => { 'folio_number' => '', 'material' => 'Paper', 'type' => 'Original', 'attached_above' => 'None',
                                                               'attached_below' => 'None', 'stub' => 'No', 'nestLevel' => 2 }, 'conjoined_leaf_order' => nil, 'parentOrder' => 2, 'rectoOrder' => 4, 'versoOrder' => 4
                                               },
                                               '5' => {
                                                 'params' => { 'folio_number' => '', 'material' => 'Paper', 'type' => 'Original', 'attached_above' => 'None',
                                                               'attached_below' => 'None', 'stub' => 'No', 'nestLevel' => 1 }, 'conjoined_leaf_order' => nil, 'parentOrder' => 1, 'rectoOrder' => 5, 'versoOrder' => 5
                                               },
                                               '6' => {
                                                 'params' => { 'folio_number' => '', 'material' => 'Paper', 'type' => 'Endleaf', 'attached_above' => 'None',
                                                               'attached_below' => 'None', 'stub' => 'No', 'nestLevel' => 1 }, 'conjoined_leaf_order' => nil, 'parentOrder' => 1, 'rectoOrder' => 6, 'versoOrder' => 6
                                               }
                                             })
        expect(export_result['Rectos']).to eq({
                                                '1' => {
                                                  'params' => { 'page_number' => '', 'texture' => 'None',
                                                                'image' => { 'manifestID' => 'DIYImages', 'label' => 'Pixel', 'url' => "https://dummy.library.utoronto.ca/images/#{@testimage.id}_pixel.png" }, 'script_direction' => 'None' }, 'parentOrder' => 1
                                                },
                                                '2' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 2 },
                                                '3' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 3 },
                                                '4' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 4 },
                                                '5' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 5 },
                                                '6' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 6 }
                                              })
        expect(export_result['Versos']).to eq({
                                                '1' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 1 },
                                                '2' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
                                                         'parentOrder' => 2 },
                                                '3' => { 'params' => { 'page_number' => '', 'texture' => 'None', 'image' => {}, 'script_direction' => 'None' },
        ...
        expect(export_result['Terms']).to eq({
                                               '1' => { 'params' => { 'title' => 'Test Term', 'taxonomy' => 'Ink', 'description' => 'This is a test', 'show' => true },
                                                        'objects' => { 'Group' => [1], 'Leaf' => [5], 'Recto' => [5],
                                                                       'Verso' => [5] } }
                                             })
    ...
    context 'for XML export' do
      ...
      it 'has expected content' do
        ...
        expect(result.css("taxonomy[xml|id='manuscript_preferences'] term").collect do |t|
                 [t['xml:id'], t.text]
               end).to include(
                 %w[manuscript_preferences_ravenna_384_2339_showTips true]
               )
    ...
    context 'with missing project' do
      before do
        get "/projects/#{@project.id}missing/export/#{@format}",
            headers: { 'Authorization' => @authToken, 'CONTENT_TYPE' => 'application/json',
                       'ACCEPT' => 'application/json' }
        @body = JSON.parse(response.body)
      end
