...
describe 'PUT /projects/:id/manifests', type: :request do
  before do
    ...
    @project = FactoryGirl.create(:project, {
                                    user: @user,
                                    manifests: { "59ee3c623b0eb75251207cfe": { id: '59ee3c623b0eb75251207cfe', name: 'ASDF',
                                                                               url: 'https://iiif.library.utoronto.ca/presentation/v2/hollar:Hollar_a_3000/manifest', images: [{ label: 'IMAGE', url: 'http://www.example.com/iiif-sample' }] } }
                                  })
  ...
  context 'with valid authorization' do
    context 'with valid parameters' do
      before do
        put "/projects/#{@project.id.to_str}/manifests", params: @parameters.to_json,
      ...
      it 'rename the manifest' do
        expect(@project.manifests.any? { |_key, manifest| manifest['name'] == 'ASDF' }).to be false
        expect(@project.manifests.any? { |_key, manifest| manifest['name'] == 'QWER' }).to be true
      end
    end

    context 'with missing project' do
      before do
        put "/projects/#{@project.id.to_str}missing/manifests", params: @parameters.to_json,
                                                                headers: { 'Authorization' => @authToken, 'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' }
      end
    ...
    context 'with unauthorized project' do
      ...
      it 'leaves the project alone' do
        expect(@project.manifests.any? { |_key, manifest| manifest['name'] == 'ASDF' }).to be true
        expect(@project.manifests.any? { |_key, manifest| manifest['name'] == 'QWER' }).to be false
      end
    end
