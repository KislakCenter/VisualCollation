...
RSpec.describe ControllerHelper::ProjectsHelper, type: :helper do
  describe 'addGroupsLeafsConjoin' do
    it 'creates a variety of groups' do
    ...
    it 'generates correct patterns of texture' do
      project = FactoryGirl.create(:project)
      addGroupsLeafsConjoin(project, [
                              { 'leaves' => 4, 'conjoin' => true },
                              { 'leaves' => 4, 'conjoin' => true },
  ...
  describe 'getManifestInformation' do
    before do
      stub_request(:get, 'https://iiif.library.utoronto.ca/presentation/v2/hollar:Hollar_a_3000/manifest').with(headers: { 'Accept' => '*/*', 'User-Agent' => 'Ruby' }).to_return(
        status: 200, body: File.read("#{File.dirname(__FILE__)}/../../fixtures/uoft_hollar.json"), headers: {}
      )
    end
    ...
    it 'pulls images' do
      ...
      expect(result[:images][1]).to eq({ label: 'Hollar_a_3000_0002',
    ...
    it 'returns the right output for the given sample' do
      ...
      expect(body[:project]).to eq({
                                     ...
                                     'manifests': { '12341234' => {
                                       ...
                                       'images' => [{ 'label' => nil, 'url' => 'https://iiif.library.villanova.edu/image/vudl%3A99215',
                                                      'manifestID' => '12341234' }]
                                     } }
                                   })
      ...
      expect(body[:rectoIDs]).to eq((@upleafs + @midleafs + @botleafs).collect(&:rectoID))
      expect(body[:versoIDs]).to eq((@upleafs + @midleafs + @botleafs).collect(&:versoID))
      expect(body[:terms]).to eq({ @testterm.id.to_s => {
                                   ...
                                   show: true,
                                   objects: { 'Group' => [@testgroup.id.to_s], 'Leaf' => [@botleafs[0].id.to_s], 'Recto' => [@botleafs[0].rectoID],
                                              'Verso' => [@botleafs[0].versoID] }
                                 } })
    end
  end
end
