...
RSpec.describe ControllerHelper::FilterHelper, type: :helper do
  ...
  it 'should reject unrecognized conjunctions' do
    ...
    expect(result).to include a_hash_including('conjunction' => 'conjunction should be one of : [OR]')
  end
  ...
  describe 'Group queries' do
    ...
    it 'should accept valid conditions for type' do
      ['equals', 'does not equal'].each do |op|
        result = runValidations([
    ...
    it 'should reject invalid conditions for type' do
      ...
      expect(result).to include a_hash_including('condition' => 'valid conditions for group attribute type : [equals, does not equal]')
    end

    it 'should accept valid parameters for title' do
      ['equals', 'does not equal', 'contains', 'does not contain'].each do |op|
        result = runValidations([
    ...
    it 'should reject invalid conditions for title' do
      ...
      expect(result).to include a_hash_including('condition' => "valid conditions for group attribute title : [equals, does not equal, contains, does not contain]")
    end
  end
  ...
  describe 'Leaf queries' do
    it 'should reject invalid attribute for leafs' do
      ...
      expect(result).to include a_hash_including('attribute' => 'valid attributes for leaf: [type, material, conjoined_to, attached_above, attached_below, stub, folio_number]')
    end

    it 'should accept valid parameters for conditions' do
      ['type', 'material', 'conjoined_to', 'attached_above', 'attached_below', 'stub'].each do |attribute|
        result = runValidations([
          { 'type' => 'leaf', 'attribute' => attribute, 'condition' => 'eq', 'values' => ['Some Value'] }
        ])
        expect(result).to include a_hash_including('condition' => "valid conditions for leaf attribute #{attribute} : [equals, does not equal]")
      end
    end
  end
  ...
  describe 'Side queries' do
    it 'should reject invalid attribute for sides' do
      ...
      expect(result).to include a_hash_including('attribute' => 'valid attributes for side: [texture, script_direction, page_number, uri]')
    end
    ...
    it 'should reject invalid conditions for texture and script_direction' do
      ['texture', 'script_direction'].each do |attribute|
        ...
        expect(result).to include a_hash_including('condition' => "valid conditions for side attribute #{attribute} : [equals, does not equal]")
      end
    end

    it 'should accept valid conditions for texture and script_direction' do
      ['texture', 'script_direction'].each do |attribute|
        ['equals', 'does not equal'].each do |condition|
          result = runValidations([
    ...
    it 'should reject invalid conditions for page_number and uri' do
      ['page_number', 'uri'].each do |attribute|
        ...
        expect(result).to include a_hash_including('condition' => "valid conditions for side attribute #{attribute} : [equals, does not equal, contains, does not contain]")
      end
    end
  ...
  describe 'Term queries' do
    it 'should reject invalid attribute for sides' do
      ...
      expect(result).to include a_hash_including('attribute' => 'valid attributes for term: [title, taxonomy, description, uri]')
    end
    ...
    it 'should reject invalid conditions for title and description' do
      ['title', 'description'].each do |attribute|
        ...
        expect(result).to include a_hash_including('condition' => "valid conditions for term attribute #{attribute} : [equals, does not equal, contains, does not contain]")
      end
    end
