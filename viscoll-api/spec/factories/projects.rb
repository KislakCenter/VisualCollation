...
FactoryGirl.define do
  ...
  factory :project do
    ...
    after(:create) do |project, evaluator|
      ...
      if evaluator.with_members.present?
  ...
  factory :codex_project, parent: :project do
    ...
    after(:create) do |project, evaluator|
      ...
      evaluator.quire_structure.each do |qs|
        qs[0].times do
          members << FactoryGirl.create(:quire, project_id: project.id, leafs: qs[1], start_page: start_page,
                                                nestLevel: 1)
          start_page += qs[1]
        end
      end
      project.add_groupIDs(members.collect { |member| member.id.to_s }, 1) if members.present?
    end
  end
