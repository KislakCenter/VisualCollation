...
RSpec.describe Term, type: :model do
  before do
    ...
    @term = FactoryGirl.create(:term, project: @project, taxonomy: ['Ink'],
                                      objects: { Group: [@group.id.to_s], Leaf: [@leaf.id.to_s], Recto: [@side1.id.to_s], Verso: [@side2.id.to_s] })
    @group.terms << @term
    @group.save
    @leaf.terms << @term
    @leaf.save
  ...
  describe 'Validations' do
    ...
    it 'is unique within the project' do
      duplicate_term = FactoryGirl.create(:term, project: @project, taxonomy: ['Ink'],
                                                 objects: { Group: [@group.id.to_s], Leaf: [], Recto: [], Verso: [] })
      duplicate_term.title = @term.title
      expect(duplicate_term).not_to be_valid
    end

    it 'does not need to be unique globally' do
      project2 = FactoryGirl.create(:project)
      group2 = FactoryGirl.create(:group, project: project2)
      project2.add_groupIDs([group2.id.to_s], 0)
      term2 = FactoryGirl.create(:term, project: project2, taxonomy: ['Ink'],
                                        objects: { Group: [group2.id.to_s], Leaf: [], Recto: [], Verso: [] })
